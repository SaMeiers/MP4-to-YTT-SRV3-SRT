document.addEventListener('DOMContentLoaded', function() {
    ...
    const exportFormatSelected = document.getElementById('exportFormatSelected');
    const exportFormatItems = document.getElementById('exportFormatItems');
    const exportFormatOptions = document.querySelectorAll('#exportFormatItems .select-option');
    let isExportFormatOpen = false;
    ...
    function initExportFormatSelect() {
        exportFormatSelected.addEventListener('click', function(e) {
            e.stopPropagation();
            isExportFormatOpen = !isExportFormatOpen;
            
            if (isExportFormatOpen) {
                exportFormatSelected.classList.add('active');
                exportFormatItems.classList.add('open');
            } else {
                exportFormatSelected.classList.remove('active');
                exportFormatItems.classList.remove('open');
            }
        });
        ...
        exportFormatOptions.forEach(option => {
            option.addEventListener('click', function() {
                const value = this.getAttribute('data-value');
                const optionName = this.querySelector('.option-name').textContent;
                const optionDesc = this.querySelector('.option-chars').textContent;
                
                exportFormatSelected.querySelector('.selected-text').textContent = `${optionName} (${optionDesc})`;
                exportFormat.value = value;
                
                exportFormatOptions.forEach(opt => opt.classList.remove('selected'));
                this.classList.add('selected');
                
                isExportFormatOpen = false;
                exportFormatSelected.classList.remove('active');
                exportFormatItems.classList.remove('open');
            });
        });
        ...
        exportFormatItems.addEventListener('click', function(e) {
            e.stopPropagation();
        });
        
        const initialExportOption = Array.from(exportFormatOptions).find(option => 
            option.getAttribute('data-value') === exportFormat.value
        );
        if (initialExportOption) {
            initialExportOption.classList.add('selected');
            const optionName = initialExportOption.querySelector('.option-name').textContent;
            const optionDesc = initialExportOption.querySelector('.option-chars').textContent;
            exportFormatSelected.querySelector('.selected-text').textContent = `${optionName} (${optionDesc})`;
        }
    }

    initCustomSelect();
    initExportFormatSelect();
